...
function Te(x)   ! eV
 ...
 if (x <= xped) then
	...
	!HeavisideTheta(1 - (-c1 + x1)/xped)*at1*(1 - ((-c1 + x1)/xped)**alt1)**alt2
 else 
	Te=tsep+at0*(tanh(2.*(1.-xmid)/Delta)-tanh(2.*((x1-c1)-xmid)/Delta))
 end if
...
!============================
!============================
!Profiles from Phys. of Plasmas 20,052505 (2013)
!Wilks, Stacey, Evans
function ne60(x)
implicit none
 real(8) ne60
 ...
 real(8) x1	
	...
	if (x <= xped) then
     ne60=nsep+an0*(tanh(2*(1-xmid)/Delta)-tanh(2*((x1-c1)-xmid)/Delta)) + &
     HeavisideTheta(1 - (x)/xped)*an1*(1 - ((x)/xped)**aln1)**aln2 
     !HeavisideTheta(1 - (-c1 + x)/xped)*an1*(1 - ((-c1 + x)/xped)**aln1)**aln2 
 else
     ne60=nsep+an0*(tanh(2*(1-xmid)/Delta)-tanh(2*((x1-c1)-xmid)/Delta))
...
implicit none
 ...
 real(8) x1
    ...
    if (x <= xped) then
	Te60=tsep+at0*(tanh(2.*(1.-xmid)/Delta)-tanh(2.*((x1-c1)-xmid)/Delta)) + &
	HeavisideTheta(1. - (x)/xped)*at1*(1. - ((x)/xped)**alt1)**alt2
	!HeavisideTheta(1 - (-c1 + x1)/xped)*at1*(1 - ((-c1 + x1)/xped)**alt1)**alt2
 else 
	Te60=tsep+at0*(tanh(2.*(1.-xmid)/Delta)-tanh(2.*((x1-c1)-xmid)/Delta))
...
Delta = 0.125
 ...
 if (x <= xped) then
	Ti60=(tsep+at0*(tanh(2*(1-xmid)/Delta) - tanh(2*((x1-c1)-xmid)/Delta))) + &
	HeavisideTheta(1 - (x)/xped)*at1*(1 - ((x)/xped)**alt1)**alt2
	!HeavisideTheta(1 - (-c1 + x)/xped)*at1*(1 - ((-c1 + x)/xped)**alt1)**alt2
 else
	Ti60=(tsep+at0*(tanh(2*(1-xmid)/Delta) - tanh(2*((x1-c1)-xmid)/Delta)))
...
function Ef60(x)
 ...
 real(8) Ef60
 real(8), intent(in) :: x
 real(8) :: a1,r1,r2,f1,be,ce,d1 !param. which define gradient 
 real(8) :: ga1, ga2, ga3,k1  !parameters, which define the Gausian 
 real(8) x1	
 x1 = x
	d1 = 20.0
	...
	ga3 = 1.4
	k1 = -0.6
 Ef60 = 1000*(d1+r2*(x1-f1)*exp(be+ce*r1*(x1-f1))*exp(-(a1*r1*(x1-f1))**2))
end function Ef60
